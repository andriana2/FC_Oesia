...
void relative2Abssolute(vector<shared_ptr<flagKnowns>> &flags)
{
    ...
    double beta = -flags.at(0)->direccion;   // Need positive rotation ccw, direccion is negative ccw
    double cosBeta = cos((M_PI/180)*beta);
    double sinBeta = sin((M_PI/180)*beta);
    double d = flags.at(0)->distancia;
    double xf = flags.at(0)->x;
    double yf = flags.at(0)->y;
